...
print.mlth.data.frame<-function(x,digits=NULL,justify='left'){
	...
	makeHeader<-function(l,digits=NULL,justify='left'){ # TODO: check that 'digits' works
		...
		if (is.list(l[[1]])){
			...
			for (i in 1:length(l[[1]])){
				subValues<-makeHeader(l[[1]][i],digits=digits,justify=justify)
				if (length(Values)==0){
					Values<-subValues
				} else {
		...
		if (length(Values)==0)
			...
			Name<-completeString(names(l),filler=ifelse(is.list(l[[1]]),'-',' '),
	...
	if (length(row.names(x))>0){
		...
		X<-list(` `=as.list(x))
		outp<-makeHeader(X,digits=digits,justify=justify)[-1]
	}
