...
#' D<-as.data.frame(D)
#' 
#' output<-lapply(D,function(x)mlth(
#' 			Parametric=list(Mean=mean(x),SD=sd(x)),
...
#' @rdname mask
#' @export
mask.default<-function(x,rows=numeric(0),cols=numeric(0),singleCells=NULL){
...
#' @rdname mask
#' @export
mask.mlth.data.frame<-function(x,rows=numeric(0),cols=numeric(0),singleCells=NULL,header=FALSE,rn=FALSE){
	computeHeight<-function(L)
		if (is.list(L)){
			max(sapply(L,computeHeight))+1
		} else 0
